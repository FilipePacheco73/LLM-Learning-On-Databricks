...
problem_raw = [
    ...
    "Broken Windshield Wipers Solution",
...
solution_raw = [
    "Jump-start the car or replace the battery",
    "Replace with a spare tire or repair the tire if possible",
    "Stop the car and allow the engine to cool down. Check coolant levels and hose connections.",
    "Use an OBD-II scanner to identify the issue and address it accordingly.",
    "Have the brake pads and discs inspected for wear and replace if necessary.",
    "Check fuel levels and replace clogged fuel filters or spark plugs if needed.",
    "Have the engine inspected for proper fuel combustion or a vacuum leak.",
    "Replace the alternator or have it repaired by a mechanic.",
    "Have the starter inspected and replaced if needed.",
    "Have the transmission fluid inspected and replaced if needed.",
    "Replace burnt-out light bulbs or have the headlights adjusted.",
    "Replace the windshield wiper blades or the entire wiper system.",
    "Have the air conditioning system inspected for leaks or low refrigerant levels.",
    "Have the exhaust system inspected for rust, holes, or loose connections.",
    "Check power steering fluid levels and replace the power steering pump or belt if needed.",
    " Have the ignition switch assembly replaced by a mechanic.",
    "Have the brake system inspected and replace worn brake pads, calipers, or master cylinder.",
    "Identify the source of the odor or noise and have it repaired by a mechanic.",
    "Check the battery, alternator, fuses, and circuitry for any faults and have them repaired.",
    "Check tire pressure, air filters, and engine oil levels and have the car serviced regularly",
]

dataset_raw = {}
for i in range(len(problem_raw)):
  dataset_raw['train'] = {'problem': problem_raw[i], 'solution': solution_raw[i]}

# COMMAND ----------

dataset_ds = Dataset.from_dict(
    {
        "problem": [
            "Dead Battery",
            "Flat Tire",
...
# dataset_ds = DatasetDict({'train': Dataset.from_dict(dataset_raw)})
dataset_ds
...
def to_tokens(tokenizer) -> callable:
    ...
    def apply(x) -> tr.tokenization_utils_base.BatchEncoding:
        ...
        """
        For a dictionary example of problem, solution of input_id and attention mask is returned
        """
        instr = x["problem"]
        resp = x["solution"]
...
print(ds_to_tokens(dataset_ds[0]))

# COMMAND ----------

print(ds_to_tokens(dataset_ds[1]))

# COMMAND ----------

#TODO
ds_to_tokens = to_tokens(tokenizer)
tokenized_dataset = dataset_ds.map(
  ds_to_tokens, batched=False, remove_columns=["problem", "solution"]
  )
tokenized_dataset
...
dataset_ds_splitted = dataset_ds.train_test_split(test_size=0.2, shuffle=True)

data_collator = tr.DataCollatorWithPadding(tokenizer=tokenizer)
trainer = tr.Trainer(
    model,
    training_args,
    train_dataset=dataset_ds_splitted["train"],
    eval_dataset=dataset_ds_splitted["test"],
    tokenizer=tokenizer,
...
final_model_path = f"/databricks/driver/llm04_fine_tuning/{checkpoint_name}"
